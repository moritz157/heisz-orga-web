...
export class AccountingService {
  ...
  getCompany(id: number){
    return new Company(this.companysJson[id]['name'], this.companysJson[id]['adress'], this.companysJson[id]['id']);
  }
  ...
  getInvoices(): Promise<Invoice[]>{
    ...
    return new Promise(function(resolve, reject){
      var result: Invoice[] = [];
      for(var i=0;i<that.invoicesJson.length;i++){
        result.push(new Invoice(that.getCompany(that.invoicesJson[i]["debtor"]), that.invoicesJson[i]["amount"], new Date(that.invoicesJson[i]["date"]), that.invoicesJson[i]["id"]));
      }
      resolve(result);
    })
  }
